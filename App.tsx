...
const App: React.FC = () => {
  ...
  // --- FUNÇÃO CRUCIAL PARA CORRIGIR O CAMINHO ---
  // Esta função pega o base URL (/oresgate/) e junta com o caminho do arquivo
  const getAssetUrl = (path: string) => {
    // Garante que não tenhamos barras duplas (ex: /oresgate//arquivos)
  ...
  // Efeito para disparar download no nível 11
  useEffect(() => {
    if (gameState.currentLevel === 11) {
      // Aqui usamos a função corrigida
      const fileUrl = getAssetUrl('arquivos/presentinho.html');
      console.log("Tentando baixar de:", fileUrl); // Para debug no console
      
      // Pequeno delay para garantir que a tela carregou
      setTimeout(() => {
        autoDownload(fileUrl, 'presentinho.html');
      }, 1000);
    }
  ...
  const renderLanding = () => (
    <div className="flex flex-col items-center justify-center min-h-[80vh] text-center px-6 animate-[fadeIn_0.8s_ease-out]">
      ...
      <div className="flex flex-col sm:flex-row gap-5 w-full max-w-lg z-20">
        <button onClick={handleStart} className="flex-1 h-20 bg-purple-600 hover:bg-purple-500 text-white font-pixel text-3xl rounded-2xl shadow-[0_6px_0_#4a1d6a] transition-all active:translate-y-1 active:shadow-none flex items-center justify-center gap-3">
          <span className="material-symbols-outlined text-3xl">play_circle</span> JOGAR
        </button>
        <button onClick={() => setShowMap(true)} className="flex-1 h-20 bg-gray-800 hover:bg-gray-700 text-white font-pixel text-3xl rounded-2xl shadow-[0_6px_0_#1a1a1a] transition-all active:translate-y-1 active:shadow-none flex items-center justify-center gap-3">
          <span className="material-symbols-outlined text-3xl">map</span> MAPA
        </button>
      </div>
      <p className="mt-12 max-w-md text-gray-500 text-xs font-bold uppercase tracking-widest">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#1c1022] border border-white/10 rounded-[2.5rem] p-8 md:p-10 shadow-2xl relative overflow-hidden">
            ...
            <div className="mb-8 space-y-4">
               <p className="text-xl md:text-2xl text-purple-50 text-center font-light italic leading-relaxed">"{r.riddle}"</p>
            </div>

            <form onSubmit={checkAnswer} className="space-y-4">
               <div className="relative">
                 <input autoFocus type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="DIGITE A CHAVE..." className="w-full bg-black/50 border-2 border-white/10 focus:border-purple-500 focus:ring-8 focus:ring-purple-500/10 rounded-2xl py-5 px-6 text-xl text-white outline-none transition-all placeholder:text-gray-700 font-pixel tracking-widest uppercase text-center"/>
                 {feedback.type && (
               ...
               <button type="submit" className="w-full py-5 bg-white text-black font-pixel text-2xl rounded-2xl hover:bg-purple-400 hover:text-white transition-all active:scale-95 flex items-center justify-center gap-3 shadow-xl">
    ...
    <div className="fixed inset-0 z-[100] bg-[#0c0611]/98 backdrop-blur-3xl flex flex-col p-6 animate-[fadeIn_0.3s_ease-out]">
      ...
      <div className="flex-1 overflow-y-auto grid grid-cols-2 sm:grid-cols-5 gap-4 max-w-5xl mx-auto w-full pb-20">
        {RIDDLES.map((r) => {
          ...
          return (
            <button key={r.id} disabled={!isUnlocked} onClick={() => handleLevelJump(r.id)}
              ...
              <span className="text-[9px] font-bold uppercase tracking-widest text-center px-2 text-white">
  ...
  const renderFinal = () => {
    // Usamos a mesma função segura para gerar o link do botão
    const fileUrl = getAssetUrl('arquivos/presentinho.html');
      ...
      <div className="flex flex-col items-center justify-center min-h-[90vh] text-center px-6 py-20 animate-[fadeIn_2s_ease-out]">
        ...
        <div className="max-w-3xl bg-white/5 border border-purple-500/20 rounded-[4rem] p-12 md:p-20 backdrop-blur-xl relative">
          ...
          <div className="flex flex-col items-center gap-4">
             {/* BOTÃO CORRIGIDO COM O BASE URL */}
             <a 
               href={fileUrl}
               download="presentinho.html"
               className="px-8 py-4 bg-yellow-500 hover:bg-yellow-400 text-black font-pixel text-xl rounded-xl shadow-lg flex items-center gap-2 animate-bounce"
             >
             ...
             <button 
               ...
               className="px-8 py-4 bg-white/5 hover:bg-white/10 rounded-xl text-gray-400 font-pixel text-sm transition-all"
             >
  ...
  return (
    <div className="min-h-screen bg-[#0c0611] text-white relative overflow-hidden">
      ...
      {gameState.currentLevel > 0 && gameState.currentLevel <= 10 && (
        <div className="fixed bottom-0 left-0 w-full h-1.5 bg-white/5 z-[60]">
          <div className="h-full bg-gradient-to-r from-purple-600 via-pink-500 to-yellow-500 transition-all duration-700 ease-out"
